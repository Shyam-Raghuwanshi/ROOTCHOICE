...
function MyApp({ Component, pageProps }) {
    ...
    if (token) {
      ...
      if (token) {
        ...
        const fetchCart = await fetch(
          `${process.env.NEXT_PUBLIC_HOST}/api/fetchcart`,
          {
            method: "POST",
          ...
          for (let i = 0; i < fetchCartData.length; i++) {
            ...
            if (slug == element.slug) {
              ...
              const res = await fetch(
                `${process.env.NEXT_PUBLIC_HOST}/api/updatecart`,
                {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify(body),
  ...
  const buynow = async (slug) => {
    ...
    let response = await fetch(
      `${process.env.NEXT_PUBLIC_HOST}/products/buynow`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
  ...
  return (
    <>
      ...
      <Navbar
        ...
        setCart={setCart}
        subtotal={subtotal}
        fetchCartFunction={fetchCartFunction}
      />
