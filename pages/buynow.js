...
const Buynow = ({ product, setSideCart }) => {
  ...
  const conform = async (e) => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_HOST}/api/pretranction`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const dependencyArrary = parseInt(router.query.q)
  useEffect(() => {
    calculateSubTotal();
  ...
  const addProduct = () => {
    if (parseInt(product.availableQty) <= parseInt(router.query.q)) {
      ...
      if (parseInt(router.query.q) >= 10) {
        toast.error("Items quantity can not be more than 10", {
          position: "bottom-center",
          autoClose: 2000,
          hideProgressBar: false,
          closeOnClick: true,
        ...
        router.push(
          `${process.env.NEXT_PUBLIC_HOST}/buynow?slug=${product.slug}&q=${
            parseInt(router.query.q) + 1
          }`
        );
      }
    }
    ...
    <>
      <div className="md:p-12 bg-gray-900 text-gray-200 mx-auto">
        ...
        <Script
          ...
          src={`${process.env.NEXT_PUBLIC_PAYTIM_HOST}/merchantpgpui/checkoutjs/merchants/${process.env.NEXT_PUBLIC_PAYTIM_MID}`}
        />
        ...
        <div className="flex flex-col w-full px-0 mx-auto md:flex-row">
          <div className="flex flex-col md:w-full">
            ...
            <form
              method="POST"
              onSubmit={paymentType !== "Cash on Delivery" ? pay : conform}
              ...
              <div className="">
                ...
                <div className="space-x-0 lg:flex lg:space-x-4 mt-5">
                  <div className="w-full lg:w-1/2">
                    ...
                    <input
                      ...
                      required
                    />
                    <span className="block mb-1 text-sm font-semibold text-gray-500">
                      Email cann&apos;t be editable
                    </span>
                  </div>
                  <div className="w-full lg:w-1/2">
                    <label
                ...
                <div className="space-x-0 lg:flex lg:space-x-4">
                  ...
                  <div className="group  w-full lg:w-1/2 inline-block">
                    ...
                    <div className=" dropdown border rounded-sm transform scale-0 group-hover:scale-100 absolute flex flex-col transition duration-150 ease-in-out origin-top bg-gray-900">
                      <span
                        onClick={() => {
                          handlePyamentType("Pay");
                        }}
                        className="rounded-sm px-3 py-1 cursor-pointer border border-b-white hover:bg-slate-400"
                      >
                        Pay
                      </span>
                      <span
                        onClick={() => {
                          handlePyamentType("Cash on Delivery");
                        }}
                        className="rounded-sm px-3 py-1 cursor-pointer hover:bg-slate-600 "
                      >
                        Cash on Delivery
                      </span>
                    </div>
                ...
                <div className="mt-4">
                  <Link passHref href="/account">
                    <button className="w-full md:w-3/12 mb-3 rounded-lg px-6 py-2 text-white bg-gray-800 active:bg-gray-800 ">
                      Change Details
                    </button>
                  </Link>
                  <button className="disabled:bg-gray-600 w-full  disabled:text-gray-500 rounded-lg px-6 py-2 text-white bg-gray-800 active:bg-gray-800 ">
                    {paymentType !== "Cash on Delivery" ? "Pay" : "Conform"}
                  </button>
                </div>
              </div>
            </form>
          ...
          <div className="mt-8 ">
            {product !== undefined && (
              <div
                key={product._id}
                className="flex  mt-14 flex-col space-y-4 border-gray-300 border-b-2 pb-2 px-10"
              >
                <div className="flex space-x-4">
                  <div>
                    <Image src={product.img} alt="image" width={112} height={112} />
                    <div className="flex mt-3 items-center justify-center text-lg">
                      <FaMinusCircle
                        onClick={removeProduct}
                        className=" cursor-pointer"
                      />
                      <span className="pl-3 pr-3">{product.quantity}</span>
                      <FaPlusCircle
                        onClick={addProduct}
                        className="cursor-pointer"
                      />
            ...
            <div className="flex flex-col py-4 px-10 text-sm font-semibold border-b border-gray-300 lg:py-5 lg:px-3 text-heading last:border-b-0 last:text-base last:pb-0">
              <span className="ml-2 mb-3">
                {" "}
                Quantity : {product !== undefined && parseInt(router.query.q)}
              </span>
              <span className="ml-2">
                Subtotal : {product !== undefined && subtotal}
              </span>
            </div>
          </div>
        </div>
